...
test('서로 다른 60개 생활사건을 제공한다', () => {
  assert.equal(journeys.length, 60);
  assert.equal(new Set(journeys.map(x => x.slug)).size, 60);
});

test('60개 사건은 8개 표준 생활분야로 탐색한다', () => {
...
test('60개 사건 모두 공식문서 심화 구조를 제공한다', () => {
  assert.equal(journeys.filter(x => x.depth === 'official-deep-dive').length, 60);
  for (const item of journeys) {
    assert.ok(item.services.length >= 5, `${item.slug}: services`);
    ...
    assert.ok(item.sources.length >= 5, `${item.slug}: sources`);
...
test('60개 사건의 제출서류 coverage는 누락 없이 현재 서비스와 일치한다', () => {
  for (const item of journeys) {
