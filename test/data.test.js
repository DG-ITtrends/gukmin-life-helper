...
test('서로 다른 66개 생활사건을 제공한다', () => {
  assert.equal(journeys.length, 66);
  assert.equal(new Set(journeys.map(x => x.slug)).size, 66);
});

test('66개 사건은 8개 표준 생활분야로 탐색한다', () => {
...
test('상세화면 전체 사건 수는 데이터 개수를 사용한다', () => {
  ...
  assert.match(mainSource, /journeys\.length/);
});

test('66개 사건 모두 공식문서 심화 구조를 제공한다', () => {
  assert.equal(journeys.filter(x => x.depth === 'official-deep-dive').length, 66);
  for (const item of journeys) {
    assert.ok(item.services.length >= 3, `${item.slug}: services`);
    assert.ok(item.eligibility?.length >= 3, `${item.slug}: eligibility`);
    assert.ok(item.deadlines?.length >= 2, `${item.slug}: deadlines`);
    assert.ok(item.benefits?.length >= 3, `${item.slug}: benefits`);
    assert.ok(item.sources.length >= 3, `${item.slug}: sources`);
    for (const entry of item.services) {
      for (const key of ['audiences','requirement','deadline','agency','channel','action','result','sourceUrl']) {
        assert.ok(entry[key], `${item.slug}/${entry.name}: ${key}`);
      }
...
test('대표 사건의 공식 근거는 일반 포털 홈이 아니라 구체 안내 또는 법령을 가리킨다', () => {
  ...
  assert.equal(featured.length, 66);
...
test('출생 사건은 국민에게 관계기관·개별서비스를 다시 찾게 하지 않는다', () => {
  ...
  const visibleText = JSON.stringify({
    ...
    services: birth.services.map(({ documentRequirements, ...service }) => service),
    steps: birth.steps,
...
test('504개 서비스 모두 유형별 제출서류와 공식 출처를 가진다', () => {
  const services = journeys.flatMap(item => item.services.map(service => ({ item, service })));
  assert.equal(services.length, 504);
  const allowed = new Set(['verified', 'verification-needed']);
  for (const { item, service } of services) {
    const docs = service.documentRequirements;
    assert.ok(docs, `${item.slug}/${service.name}: documentRequirements`);
    for (const key of ['required', 'conditional', 'autoChecked', 'outputs']) assert.ok(Array.isArray(docs[key]), `${item.slug}/${service.name}: ${key}`);
    assert.ok(allowed.has(docs.verificationStatus), `${item.slug}/${service.name}: verificationStatus`);
    assert.match(docs.sourceUrl, /^https:\/\//, `${item.slug}/${service.name}: sourceUrl`);
  }
});

test('66개 사건의 제출서류 coverage는 누락 없이 현재 서비스와 일치한다', () => {
  for (const item of journeys) {
    assert.equal(item.documentCoverage.total, item.services.length, `${item.slug}: total`);
    assert.equal(item.documentCoverage.mapped, item.services.length, `${item.slug}: mapped`);
  }
});

test('창업 허브 뒤에 10개 업종별 창업 사건과 제출서류가 연결된다', () => {
  const slugs = ['restaurant-startup','cafe-bakery-startup','beauty-barber-startup','online-store-startup','academy-startup','retail-store-startup','lodging-minbak-startup','sports-facility-startup','pet-service-startup','real-estate-office-startup'];
  const hubIndex = journeys.findIndex(x => x.slug === 'startup');
  assert.equal(journeys[hubIndex].title, '창업 유형을 선택해요');
  assert.deepEqual(journeys.slice(hubIndex + 1, hubIndex + 11).map(x => x.slug), slugs);
  for (const slug of slugs) {
    const item = journeys.find(x => x.slug === slug);
    assert.ok(item.services.length >= 6, slug);
    assert.equal(item.documentCoverage.mapped, item.services.length, slug);
  }
});

test('채무지원 허브 뒤에 6개 상황별 사건과 제출서류가 연결된다', () => {
  const slugs = ['debt-before-arrears','debt-financial-arrears','debt-small-business','debt-personal-rehabilitation','debt-bankruptcy-discharge','debt-illegal-collection'];
  const hubIndex = journeys.findIndex(x => x.slug === 'debt-crisis');
  assert.deepEqual(journeys.slice(hubIndex + 1, hubIndex + 7).map(x => x.slug), slugs);
  for (const slug of slugs) {
    const item = journeys.find(x => x.slug === slug);
    assert.ok(item.services.length >= 4, slug);
    assert.equal(item.documentCoverage.mapped, item.services.length, slug);
  }
});
