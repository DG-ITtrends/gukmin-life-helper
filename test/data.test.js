...
test('서로 다른 20개 생활사건을 제공한다', () => {
  assert.equal(journeys.length, 20);
  assert.equal(new Set(journeys.map(x => x.slug)).size, 20);
});
...
test('20개 사건 모두 대표 3개와 같은 공식문서 심화 구조를 제공한다', () => {
  assert.equal(journeys.filter(x => x.depth === 'official-deep-dive').length, 20);
