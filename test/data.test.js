...
test('50개 사건 모두 공식문서 심화 구조를 제공한다', () => {
  assert.equal(journeys.filter(x => x.depth === 'official-deep-dive').length, 50);
...
test('대표 사건의 공식 근거는 일반 포털 홈이 아니라 구체 안내 또는 법령을 가리킨다', () => {
  ...
  assert.equal(featured.length, 50);
...
test('50개 사건의 제출서류 coverage는 누락 없이 현재 서비스와 일치한다', () => {
  for (const item of journeys) {
